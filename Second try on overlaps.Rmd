...
gr_cad_100 <- gr_cad
start(gr_cad_100) <- pmax(1, start(gr_cad) - 100000)
end(gr_cad_100) <- end(gr_cad) +100000
...
hits_100k <- findOverlaps(gr_cad_100, gr_pqtl) #is this a problem?
...
genes_nonspecific <- c("ST3GAL4","DPAGT1", "GALNT4", "B3GNT3", "B3GNT8", "FUT9", "ST3GAL5", "B4GALNT2")
 ...
 for (gene in genes_nonspecific) {
  gene_targets_nonspecific <- data_filt_glyenz %>%
    filter(enz_gene == gene) %>%
    distinct(Gene_name)
  
common_targets_nonspecific <-
  intersect(gene_targets_nonspecific$Gene_name, common_overlap_target_100k)
  #prioritise the ones targeted witht he specific gene. 
