...
cad_for_coloc <- read.table(".\\data\\raw\\coloc_enz_genes\\B4GALNT2_for_coloc.txt", header = TRUE, sep = "", stringsAsFactors = FALSE, fill = TRUE)
...
```{r}
protein_for_coloc <- as.data.frame(read.table(".\\data\\raw\\coloc_enz_genes\\UMOD_UKBPPP.txt", header = TRUE, sep = "", stringsAsFactors = FALSE, fill = TRUE))
...
protein_for_coloc$Allele1 <- tolower(protein_for_coloc$Allele1)
protein_for_coloc$Allele2 <- tolower(protein_for_coloc$Allele2)
```
